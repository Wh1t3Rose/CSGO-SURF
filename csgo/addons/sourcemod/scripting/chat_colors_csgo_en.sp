...
public Action CP_OnChatMessage(int& author, ArrayList recipients, char[] flagstring, char[] name, char[] message, bool& processcolors, bool& removecolors)
{
	if(colors[author])
	{
		Format(name, MAXLENGTH_NAME, " %s%s%s%s", tag_Color[author], tag[author], name_Color[author], name);
		Format(message, (MAXLENGTH_MESSAGE - strlen(name) - 5), "%s%s", chat_Color[author], message);
		processcolors = false;
